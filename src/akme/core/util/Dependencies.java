 ...
 * Use <code>V get(Class&lt;V&gt; key, V default)</code> to avoid casting and string names. This is intended to work
 * with constructor injection that sets private final dependency properties, similar to Google Guice but just in one
 * simple class extending Properties and supporting obtaining Singletons and, via putCallableWrapped(), Prototypes.
 */
public class Dependencies extends Properties {
	...
	/**
	 * Get by key also trying any defaults given to the constructor. If the value is found, other than the given
	 * defaultValue, and that value is Callable, the result of the Callable.call() will be returned rather than the
	 * Callable itself. Throws an IllegalStateException if the dependency is missing/null.
	 */
	@Override
	...
	/**
	 * Get by key also trying any defaults given to the constructor. Avoids casting if given a Class as key. If the
	 * value is found, other than the given defaultValue, and that value is Callable, the result of the Callable.call()
	 * will be returned rather than the Callable itself. Throws an IllegalStateException if the dependency is
	 * missing/null.
	 */
	@SuppressWarnings("unchecked")
	public <V> V get(Class<V> key) {
		// Cast to Object to avoid a recursive cycle.
		final Object okey = key;
		return (V) get(okey);
	}

	/**
	 * Get by key also trying any defaults given to the constructor, and if still null then the given default. Avoids
	 * casting if given a default value. If the value is found, other than the given defaultValue, and that value is
	 * Callable, the result of the Callable.call() will be returned rather than the Callable itself. Throws an
	 * IllegalStateException if the dependency is missing/null.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Get by key also trying any defaults given to the constructor, and if still null then the given default. Avoids
	 * casting if given a Class as key and a compatible defaultValue. If the value is found, other than the given
	 * defaultValue, and that value is Callable, the result of the Callable.call() will be returned rather than the
	 * Callable itself. Throws an IllegalStateException if the dependency is missing/null.
	 */
	...
	/**
	 * Put the given value in the Dependencies with key by its class, i.e. value.getClass(). Throws NullPointerException
	 * if value is null.
	 */
	...
	/**
	 * Put the given Callable into the dependencies wrapped in another callable such that a get(key) will return the
	 * original Callable itself. This simplifies setting and getting a Callable itself rather than getting what it
	 * returns.
	 */
	...
	/**
	 * Put the given Supplier into the dependencies wrapped in another supplier such that a get(key) will return the
	 * original Supplier itself. This simplifies setting and getting a Supplier itself rather than getting what it
	 * returns.
	 */
